...
// ============================================================
async function stageWorkspace(ctx: PipelineContext): Promise<void> {
  ...
  if (contents.length > 0) throw new Error("Workspace not empty after cleanup");
...
// ============================================================
async function stageSource(ctx: PipelineContext): Promise<void> {
  const dir = ctx.workspaceDir;

  if (ctx.repoUrl) {
    if (!ctx.branch) ctx.branch = "main";
    ...
    // Verify
    if (!fs.existsSync(path.join(dir, ".git"))) throw new Error("Git clone failed: no .git directory");
    const files = fs.readdirSync(dir).filter(f => f !== ".git");
    if (files.length === 0) throw new Error("Git clone produced empty repository");
    ctx.onLog("✓ Repository cloned successfully");
  } else if (ctx.zipPath) {
    if (!fs.existsSync(ctx.zipPath)) throw new Error(`Zip file not found: ${ctx.zipPath}`);
    ...
    if (extracted.length === 0) throw new Error("Zip extraction produced no files");
...
// ============================================================
async function stageDetect(ctx: PipelineContext): Promise<void> {
  const dir = ctx.workspaceDir;
  const pkgPath = path.join(dir, "package.json");

  if (!fs.existsSync(pkgPath)) throw new Error("Not a Node.js project: no package.json found");
  ...
  if (!hasNext) throw new Error("Not a Next.js project: 'next' not found in dependencies");
  ...
  if (!pkg.scripts || !pkg.scripts.build) throw new Error("Missing build script in package.json");
  if (!pkg.scripts || !pkg.scripts.start) throw new Error("Missing start script in package.json");
...
// ============================================================
async function stageSecurity(ctx: PipelineContext): Promise<void> {
  ...
  if (!result.passed) {
    ...
    throw new Error(
      `Security check failed: ${result.issues.length} issue(s), ` +
...
async function stageInstall(ctx: PipelineContext): Promise<void> {
      ...
      if (packages.length > 0) {
        ...
        try {
          ...
          try {
            ...
            if (!hadExistingWs) {
              try { fs.unlinkSync(wsPath); } catch {}
            }
          }
        }
        ...
        throw new Error(
          "pnpm install failed with ERR_PNPM_IGNORED_BUILDS but could not parse blocked package names. " +
  ...
  if (!fs.existsSync(nextBin)) throw new Error("Install failed: next binary not found in node_modules/.bin/next");
...
// ============================================================
async function stageBuild(ctx: PipelineContext): Promise<void> {
  const dir = ctx.workspaceDir;
  ...
  if (!fs.existsSync(nextDir)) throw new Error("Build failed: .next directory not found");
...
// ============================================================
async function stageEnv(ctx: PipelineContext): Promise<void> {
  const dir = ctx.workspaceDir;
...
// ============================================================
async function stagePM2(ctx: PipelineContext): Promise<void> {
  const dir = ctx.workspaceDir;
  const nextBin = path.join(dir, "node_modules", ".bin", "next");
  const appName = "nextapp";

  // Stop existing if any
  try { await runCmd("pm2", ["stop", appName], undefined, 10000); } catch {}
  try { await runCmd("pm2", ["delete", appName], undefined, 10000); } catch {}
  ...
  fs.writeFileSync(path.join(ecoDir, "ecosystem.config.js"),
  ...
  function readPm2ErrorLog(): string {
    try {
  ...
  for (let i = 1; i <= MAX_POLLS; i++) {
    ...
    const app = list.find((p: any) => p.name === appName);
    if (!app) {
    ...
    if (lastRestartCount >= 0 && restartCount > lastRestartCount) {
      ...
      if (crashLoopStreak >= 3) {
        ...
        throw new Error(
          `Application is crashing after startup (${restartCount} restarts).\n` +
          reason
        );
      }
    }
  ...
  const crashMsg = lastRestartCount > 0
    ...
    : "";
  throw new Error(
    `PM2 startup timeout after ${MAX_POLLS} seconds.` +
    crashMsg +
...
// ============================================================
async function stageNginx(ctx: PipelineContext): Promise<void> {
  ...
  await setupNginx((msg: string) => ctx.onLog(msg), ssl);
}

// ============================================================
// STAGE 10: Health — Verify the app is running
// ============================================================
async function stageHealth(ctx: PipelineContext): Promise<void> {
  ...
  const app = list.find((p: any) => p.name === "nextapp");
  if (!app) throw new Error("Health check failed: PM2 process not found");
  if (app.pm2_env?.status !== "online") throw new Error(`Health check failed: PM2 status is ${app.pm2_env?.status}`);
