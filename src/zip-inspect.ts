...
function readCentralEntries(fd: number, size: number): { fileName: string; compressedSize: number; uncompressedSize: number }[] {
  ...
  for (let i = maxSearch - 22; i >= 0; i--) {
    if (readU32(buf, i) === EOCD_SIG) { eocdOffset = size - maxSearch + i; break; }
  }
  ...
  if (numEntries > MAX_ENTRY_COUNT) throw new Error(`Zip bomb detected: ${numEntries} entries (max ${MAX_ENTRY_COUNT})`);
  ...
  const entries: { fileName: string; compressedSize: number; uncompressedSize: number }[] = [];
  let off = 0;
  for (let i = 0; i < numEntries; i++) {
    if (readU32(cdBuf, off) !== CENTRAL_DIR_ENTRY_SIG) throw new Error(`Invalid ZIP: bad central directory entry at index ${i}`);
...
export async function inspectZip(zipPath: string): Promise<ZipInspectResult> {
  ...
  try {
    ...
    if (entries.length === 0) throw new Error("ZIP file contains no entries");
    if (entries.length > MAX_ENTRY_COUNT) throw new Error(`Zip bomb detected: ${entries.length} entries (max ${MAX_ENTRY_COUNT})`);
    ...
    if (suspicious) throw new Error(suspicious);
