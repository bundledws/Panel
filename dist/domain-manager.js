...
import { createDomain, getDomain, getDomainByDomain, listDomains, updateDomainState, updateDomain, deleteDomain, logAudit, createAcmeOrder, updateAcmeOrder, createDnsVerification, createCertificateRenewal, updateCertificateRenewal, getCertificateRenewal, } from "./domain-store.js";
import { validateDomain, verifyDnsResolution, pollDnsUntilVerified, generateDnsInstructions, isPrivateIp, detectProxyStatus, } from "./domain-validate.js";
import { generatePerDomainConfig, writeDomainConfig, removeDomainConfig, withNginxLock, testNginxConfig, reloadNginx, ensureSnippets, } from "./nginx.js";
import { issueCertificateHttp01, issueCertificateDns01, renewCertificate, cleanupChallenge, calculateNextRenewal, needsRenewal, isExpired, } from "./certificate.js";
const APP_PORT = 3000;
...
export class DomainManager {
     ...
     */
    async pollDns(domainId, maxAttempts = 30, intervalMs = 10000) {
        const domain = getDomain(domainId);
        if (!domain)
            throw new Error("Domain not found");
    ...
    async configureNginx(domainId) {
        ...
        return withNginxLock(async () => {
            try {
                ...
                const config = generatePerDomainConfig(domain.domain, APP_PORT, null, // No cert yet
                null, // No key yet
                ACME_CHALLENGE_DIR);
     ...
     */
    async issueCertificate(domainId) {
        const domain = getDomain(domainId);
        if (!domain)
            throw new Error("Domain not found");
        try {
            ...
            if (effectiveChallengeType === "dns-01") {
                ...
                if (domain.dnsProvider === "cloudflare" || domain.proxyStatus === "proxied") {
                    ...
                    if (cfToken && !cfZoneId) {
                        try {
                            ...
                            const zoneResp = await fetch(`https://api.cloudflare.com/client/v4/zones?name=${apexDomain}`, { headers: { Authorization: `Bearer ${cfToken}` } });
                ...
                if (cfToken && cfZoneId) {
                    ...
                    result = await issueCertificateDns01(domain.domain, cfToken, cfZoneId, (msg) => this.log(domainId, msg));
                }
                else {
                    ...
                    result = await issueCertificateHttp01(domain.domain, (msg) => this.log(domainId, msg));
                    ...
                    if (!result.success) {
                        ...
                        result = await issueCertificateDns01(domain.domain, "", // empty token triggers manual mode
            ...
            updateDomain(domainId, { certChallengeType: effectiveChallengeType });
    ...
    async enableHttps(domainId) {
        ...
        return withNginxLock(async () => {
            try {
                ...
                const config = generatePerDomainConfig(domain.domain, APP_PORT, domain.certPath, domain.keyPath, ACME_CHALLENGE_DIR);
     ...
     */
    async scheduleRenewal(domainId) {
        const domain = getDomain(domainId);
        if (!domain)
            throw new Error("Domain not found");
        if (!domain.certNotAfter)
            throw new Error("Certificate expiry not set");
        try {
    ...
    async checkRenewals() {
        ...
        for (const domain of domains) {
            if (domain.state !== "active" && domain.state !== "https_active")
                continue;
            if (!domain.certNotAfter)
                continue;
    ...
    async retryDomain(domainId) {
        ...
        if (domain.state !== "failed")
            throw new Error(`Cannot retry domain in state: ${domain.state}`);
     ...
     */
    async removeDomain(domainId, actor) {
        const domain = getDomain(domainId);
        if (!domain)
            throw new Error("Domain not found");
        return withNginxLock(async () => {
     ...
     */
    async verifyHealth(domainId) {
        const domain = getDomain(domainId);
        if (!domain)
            return { healthy: false, issues: ["Domain not found"] };
        const issues = [];
