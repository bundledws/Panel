...
// ============================================================
export function findAcmeSh() {
    return fs.existsSync(ACME_BIN);
}
export async function ensureAcmeSh() {
    if (findAcmeSh())
        return true;
    try {
...
// Certificate Paths
// ============================================================
export function getCertStorageDir(domain) {
 ...
 */
export function findAcmeCertDir(domain) {
 ...
 */
export async function verifyCertificate(certPath, keyPath) {
 ...
 */
export async function issueCertificateHttp01(domain, onLog) {
    ...
    try {
        ...
        const result = await runCmd(ACME_BIN, [
            "--issue",
            "-d", domain,
            "-d", wwwDomain,
        ...
        ], undefined, 120000, (msg) => { output.push(msg); log(msg); });
 ...
 */
export async function issueCertificateDns01(domain, cfToken, cfZoneId, onLog) {
    const log = (msg) => { if (onLog)
        onLog(msg); };
    const output = [];
    try {
        ...
        const result = await runCmd(ACME_BIN, [
            ...
            "--server", "letsencrypt",
            "--force",
        ], undefined, 180000, // DNS-01 can take longer due to propagation
        (msg) => { output.push(msg); log(msg); }, env);
        output.push(result.stdout);
        log("Certificate issued successfully.");
        // Find the acme.sh cert directory
        const acmeCertDir = findAcmeCertDir(domain);
 ...
 */
export async function installCertificate(domain, acmeCertDir) {
    ...
    try {
        fs.chmodSync(certPaths.privkey, 0o600);
    }
    catch { }
    try {
        fs.chmodSync(certPaths.fullchain, 0o644);
    }
    catch { }
    return certPaths;
}
 ...
 */
export async function renewCertificate(domain, onLog) {
    const log = (msg) => { if (onLog)
        onLog(msg); };
    const output = [];
    try {
        ...
        const result = await runCmd(ACME_BIN, ["--renew", "-d", domain, "--server", "letsencrypt", "--force"], undefined, 120000, (msg) => { output.push(msg); log(msg); });
        output.push(result.stdout);
        // Find the acme.sh cert directory
        const acmeCertDir = findAcmeCertDir(domain);
        if (!acmeCertDir) {
 ...
 */
export async function cleanupChallenge(domain) {
    ...
    if (fs.existsSync(challengeDir)) {
        try {
            ...
            for (const file of files) {
                try {
                    fs.unlinkSync(path.join(challengeDir, file));
                }
                catch { }
            }
        }
...
export function shouldRetry(error, attempt, maxRetries) {
    ...
    for (const pattern of nonRetryable) {
        if (lower.includes(pattern)) {
            ...
            if (pattern === "rate limit exceeded" && attempt < 3)
                return true;
            return false;
        }
    }
 ...
 */
export function calculateNextRenewal(notAfter) {
 ...
 */
export function needsRenewal(notAfter) {
 ...
 */
export function isExpired(notAfter) {
