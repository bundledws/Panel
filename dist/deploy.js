...
export async function restartWithRebuild(onLog) {
    ...
    if (fs.existsSync(pkgPath)) {
        ...
        if (pkg.scripts && pkg.scripts.build) {
            if (pm === "pnpm")
                await runCmd("pnpm", ["run", "build"], appDir, 300000);
            else if (pm === "yarn")
                await runCmd("yarn", ["build"], appDir, 300000);
            else
                await runCmd("npm", ["run", "build"], appDir, 300000);
        }
        log("Build complete.");
    }
    // Start the app
...
export async function getAppStatus() {
    try {
        ...
        const app = list.find((p) => p.name === "nextapp" || p.name === "bundledws-app");
        if (!app)
...
export async function getAppLogs(lines = 100) {
...
export async function getLogsSince(hours) {
    ...
    for (const name of ["nextapp", "bundledws-app"]) {
        try {
            ...
            const lines = result.stdout.split("\n").filter(l => {
                ...
                if (match)
                    return new Date(match[0]).getTime() >= since;
