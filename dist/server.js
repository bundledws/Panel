...
import { readIntEnv, getAppDir } from "./env.js";
...
import { generateDnsInstructions } from "./domain-validate.js";
import { listConfiguredDomains } from "./nginx.js";
import { getDomain, listDomains, getAuditLogs, getDnsVerifications, getAcmeOrder, getDomainStats } from "./domain-store.js";
...
const appActivityLogs = new Array(MAX_ACTIVITY_LOGS);
let logIndex = 0;
let logCount = 0;
function logActivity(msg) {
    ...
    if (logCount < MAX_ACTIVITY_LOGS)
        logCount++;
}
function getActivityLogs() {
    if (logCount < MAX_ACTIVITY_LOGS)
        return appActivityLogs.slice(0, logCount);
...
function checkRateLimit(ip) {
    ...
    if (!b) {
        b = { tokens: RATE_LIMIT.maxTokens, lastRefill: now };
        rateBuckets.set(ip, b);
    }
    const elapsed = now - b.lastRefill;
    const refill = Math.floor(elapsed / RATE_LIMIT.refillMs) * RATE_LIMIT.refillRate;
    if (refill > 0) {
        b.tokens = Math.min(RATE_LIMIT.maxTokens, b.tokens + refill);
        b.lastRefill = now;
    }
    if (b.tokens <= 0)
        return false;
...
function isBehindHttpsProxy(req) {
...
function requireCsrf(req, p) {
    const csrf = readCsrf(req);
    if (!csrf || csrf !== p.session.csrfToken)
...
function validateRepoUrl(url) {
    if (!/^(https?:\/\/|git@)[a-zA-Z0-9._:/%-]+\.git$/.test(url))
...
function route(method, path, def) {
...
route("POST", "/api/setup", {
    handler: async (req, res, body) => {
        if (await cp.hasAdmin())
            return sendError(res, 400, "Already set up");
        const { email, password } = body;
        if (!email || !password || password.length < 8)
            return sendError(res, 400, "Email and password (min 8 chars) required");
...
route("POST", "/api/deploy/git", {
    ...
    handler: async (req, res, body) => {
        ...
        if (!repoUrl)
            return sendError(res, 400, "repoUrl required");
        validateRepoUrl(repoUrl);
        const userName = process.env.USER || process.env.LOGNAME || "ubuntu";
        const workspaceDir = `/home/${userName}/Myapp`;
        const logLines = [];
        const onLog = (line) => { logLines.push(line); logActivity(line); };
...
route("POST", "/api/deploy/zip", {
    ...
    handler: async (req, res, body, ip) => {
        ...
        if (file.data.length > 50 * 1024 * 1024)
            return sendError(res, 400, "File exceeds 50MB limit");
...
route("POST", "/api/app/restart", {
    ...
    handler: async (req, res) => {
        ...
        const output = await restartWithRebuild((msg) => { restartLogs.push(msg); logActivity(msg); });
...
route("POST", "/api/app/env/delete", {
    ...
    handler: async (req, res, body) => {
        const { key } = body;
        if (!key)
            return sendError(res, 400, "key required");
        ...
        if (fs.existsSync(envPath)) {
            ...
            for (const line of content.split("\n")) {
                const eq = line.indexOf("=");
                if (eq > 0)
                    existing[line.substring(0, eq)] = line.substring(eq + 1);
            }
        }
        delete existing[key];
...
route("PUT", "/api/app/env", {
    ...
    handler: async (req, res, body) => {
        ...
        if (!env || typeof env !== "object")
            return sendError(res, 400, "env must be an object");
        const myappDir = getAppDir();
        fs.mkdirSync(myappDir, { recursive: true });
        const envPath = `${myappDir}/.env`;
        let existing = {};
        ...
        fs.writeFileSync(pathMod.join(profileDir, "bundledws.sh"), profileLines.join("\n") + "\n", "utf8");
        try {
            fs.chmodSync("/etc/profile.d/bundledws.sh", 0o644);
        }
        catch { }
        try {
        ...
        const restartLogs = [];
        try {
            await restartWithRebuild((msg) => { restartLogs.push(msg); logActivity(msg); });
        }
        catch {
            restartLogs.push("Restart with rebuild failed.");
            logActivity("Restart with rebuild failed.");
        }
...
route("POST", "/api/domains", {
    ...
    handler: async (req, res, body, ip) => {
        ...
        if (!domain)
            return sendError(res, 400, "domain required");
        if (!mode)
            return sendError(res, 400, "mode required (customer_dns or managed_dns)");
...
http.createServer(async (req, res) => {
    ...
    try {
        // Static files
        if (method === "GET" && serveStatic(req, res, path))
            return;
        ...
        if (domainMatch && domainMatch.domainId) {
            ...
            if (method === "POST" && action === "request-cert") {
                ...
                if (domain.state === "dns_verified") {
                    ...
                    if (!nginxOk)
                        return sendError(res, 400, "Nginx configuration failed");
                }
